...
if __name__ == "__main__":
    ...
    print(f"Total execution cost: {exec_res['execution_cost']:.4f}")
    print("First 3 segments exec cost:")
    for info in exec_res["per_segment_costs"]:
        print(f"  Seg {info['segment_idx']}: exec={info['execution_cost']:.4f}")
    ...
    print(f"Total idle cost: {idle_res['idle_cost']:.4f}")
    print("First 3 segments idle cost:")
    for info in idle_res["per_segment_costs"]:
        print(f"  Seg {info['segment_idx']}: idle={info['idle_cost']:.4f}")
    ...
    print(f"Total movement cost: {move_res['movement_cost']:.4f}")
    print("First 3 segments movement cost:")
    for info in move_res["per_segment_costs"]:
        print(
            f"  Seg {info['segment_idx']}: move={info['movement_cost']:.4f}"
        )
    ...
    print(f"Execution cost: {cost_all['execution_cost']:.4f}")
    print(f"Idle cost:      {cost_all['idle_cost']:.4f}")
    print(f"Movement cost:  {cost_all['movement_cost']:.4f}")
    print(f"Total cost:     {cost_all['total_cost']:.4f}")
