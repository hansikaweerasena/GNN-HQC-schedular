...
- **Signature `sinkhorn(Z, caps)` with `Z: [..., R, K]`** — all reductions on the
  last two dims. Single-circuit is the no-leading-dims case, so the operator
  encodes no batching assumption and is unchanged if ragged batching ever
  arrives; a fixed-$N$ batch passes `[B,R,K]` and runs vectorised. A per-circuit
  Python loop measures ~16× slower here (dispatch-bound, not FLOP-bound) —
  roughly 7 h added per 140-epoch run.
- Large finite negative constant for masked entries, never `-inf`.
...
2. Hardener burden ≈ 0 late in training?
3. EFCL converges?
...
- $N = 30..39$, **stratified equally**: 120 training circuits per size.
- **Depth selection within each $N$ group, not globally.** A global filter lets
