...
MODEL_CFG = {
    # Input/feature dims — node_feat_dim is the BASE width and must match
    # qubit_interaction_graph.NODE_FEAT_DIM. The EFFECTIVE width is
    # node_feat_dim(global_features): 16 normally, 18 with the flag set.
    # Build models from that helper, never from this number directly.
    "node_feat_dim":  16,
    "edge_feat_dim":  5,

    # Remaining-horizon global node features (future_idle_fraction,
    # future_2q_fraction). Adds 2 columns -> node_feat_dim 16 -> 18.
    # Overridable with --global_features. Run ON and OFF as a paired array:
    # the N=30 pilot is NOT a valid control (different temperature_gamma and
    # sinkhorn_iters), so an unpaired comparison confounds three changes.
