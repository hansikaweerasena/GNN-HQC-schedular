...
For fixed-topology technology capacity kappa,
    ...
    B_dense        = max(0, C_raw - kappa) / (kappa + eps)
...
def compute_dense_gate_rows(
    ...
    for s, layer in enumerate(motif.layers):
        ...
        for gate_idx, pair in enumerate(ordered_pairs, start=1):
            ...
            B_dense = max(0.0, C_raw - float(kappa)) / (float(kappa) + cfg.eps)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--kappas", type=str, default="3", help="Comma-separated kappa values, e.g. 2,3,4")
