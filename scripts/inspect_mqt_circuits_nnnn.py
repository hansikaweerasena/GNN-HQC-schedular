...
def _reverse_qubit_order(qc: QuantumCircuit) -> QuantumCircuit:
    """
    Rebuild circuit with reversed qubit numbering:  q_i  →  q_{N-1-i}.

    All gates are remapped accordingly. Classical registers (if any) are
    preserved and measurement mappings are reversed as well.
    """
    N = qc.num_qubits
    ...
    for instruction in qc.data:
        ...
        # Remap qubit indices:  qubit at position i  →  position N-1-i
        new_qargs = [qc.qubits[N - 1 - qc.qubits.index(q)] for q in qargs]
...
def parse_args():
    ...
    p.add_argument("--reverse_qubits", action="store_true",
                   help="Reverse qubit ordering: q_i -> q_{N-1-i} (visual only)")
    return p.parse_args()
...
def main():
    ...
    # ── Optional: reverse qubit ordering ──
    if args.reverse_qubits:
        log("Reversing qubit ordering: q_i -> q_{N-1-i}")
        mqt_circuits = [
            (algo, nq, _reverse_qubit_order(qc), _reverse_qubit_order(qc_raw))
