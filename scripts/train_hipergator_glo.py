...
from src.qubit_interaction_graph import (
    ...
    NODE_FEAT_DIM,
    node_feat_dim as compute_node_feat_dim,
    EDGE_FEAT_DIM,
)
...
def parse_args():
    ...
    p.add_argument("--global_features", action="store_true", default=None,
                   help="Append the two remaining-horizon node features "
                        "(future_idle_fraction, future_2q_fraction), giving "
                        "node_feat_dim=18 instead of 16. Overrides MODEL_CFG. "
                        "Run this ON and OFF as a paired array: the N=30 pilot "
                        "is NOT a valid control (different temperature_gamma "
                        "and sinkhorn_iters), so an unpaired comparison "
                        "confounds three changes at once.")
...
def build_layer_data_list_cpu(circuit, w_short: int, w_long: int,
...
class CircuitDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # Always build on CPU
        layer_data_list = build_layer_data_list_cpu(
            rep, self.w_short, self.w_long, self.global_features)
...
def save_model_arch_params(evol_model: EvolvingGNN, cluster_module: SegmentClustering,
    ...
    arch = {
        "EvolvingGNN": {
            ...
            # Needed at EVAL time, not merely for the record. node_feat_dim
            # above tells eval how WIDE the model is; it does not tell eval
            # that it must BUILD 18 features. Without this key eval would
            # construct an 18-wide model and feed it 16 columns. Absent key
            # => False, matching every run that predates the flag.
            "global_features": bool(global_features),
        },
        "SegmentClustering": {
...
def main():
    ...
    # Effective node feature width is derived here ONCE, from the flag, and
    # every downstream consumer (dataset assertion, DERIVED block, model
    # construction, arch params) reads this single value. Deriving it instead
    # from the first sample would make the assertion below vacuous; hardcoding
    # it in two places would let them drift.
    MODEL_CFG = dict(MODEL_CFG)
    if args.global_features is not None:
        MODEL_CFG["global_features"] = args.global_features
    global_features   = bool(MODEL_CFG.get("global_features", False))
    eff_node_feat_dim = compute_node_feat_dim(global_features)
    ...
    derived = {
        ...
        "w_short":           w_short,
        "w_long":            w_long,
        "node_feat_dim":     eff_node_feat_dim,
        "global_features":   global_features,
    ...
    # Guards the one failure here that would otherwise be silent: the flag set
    # to True while build_layer_graph_arrays did not actually append the
    # columns. Compares the REAL array width against the width derived from
    # the flag, so a stale feature builder cannot slip through as a quietly
    # different model.
    _probe_x = train_dataset[0][0][0].x
    assert _probe_x.shape[1] == eff_node_feat_dim, (
        f"node feature width mismatch: dataset produced {_probe_x.shape[1]}, "
        f"config expects {eff_node_feat_dim} "
        f"(global_features={global_features}). Check that "
        f"build_layer_graph_arrays received the flag."
    )
    log(f"Node features   : dim={eff_node_feat_dim}  "
